...
class MultiProviderEvaluator:
    ...
    def __init__(self, difficulty: str = "normal", seed_index: int = 0, 
        ...
        self.num_episodes = num_episodes  # New: number of episodes
        ...
        # New: create subdirectories for each episode
        self.episode_dirs = {}
        if self.save_media:
        ...
        # Do not create environment during initialization, it will be created at the start of each episode
        self.env = None
        
        # Results tracking - modified to support multiple episodes
        self.results = {
            "config": {
                ...
                "vision_support": True,
                "num_episodes": num_episodes,  # New
                "timestamp": datetime.now().isoformat(),
                "output_directory": str(self.output_dir) if self.save_media else None
            },
            "episodes": [],  # Modified: store results for each episode
            "summary_stats": {},  # New: summary statistics
            "media_files": {
                "images": [],
                "audio": [],
                "videos": [],
        ...
        # Enhanced system prompt - optimized for multi-episode evaluation
...
"""
        ...
        if self.input_mode == "video":
            if self.enhanced_video:
                media_description = f"""- Video: Visual sequence with integrated audio (FPS: {video_fps}, Audio per frame: {self.audio_duration_per_frame}s)
...
"""
    ...
    def _build_messages(self, observation, step: int, video_path: Optional[str] = None, 
        ...
        """Build a message containing multimodal content"""
        try:
            # Build base text content
            if self.include_vector_text:
...
Analyze the situation and provide your ONE command."""
            ...
            # 🎯 Modified: Process video data - extract frames and convert to image sequence
            if isinstance(observation, dict) and observation.get('video'):
                ...
                if isinstance(video_data, str) and video_data:
                    try:
                        # Detect video format and extract frames
                        ...
                        if is_actual_video and MOVIEPY_AVAILABLE:
                            # It's a real video, start frame extraction
                            print(f"🎬 Video data detected, starting frame extraction process...")
                            ...
                            if video_frames:
                                ...
                                print(f"✅ Added {len(video_frames)} video frames to the message")
                            else:
                                print("⚠️ Video frame extraction failed, falling back to single frame processing")
                                # Fallback to single frame image
                                content.append({
                                    "type": "image_url",
                                    "image_url": {
                                ...
                                logger.info("✅ Fallback image data added to the message")
                        elif is_actual_video and not MOVIEPY_AVAILABLE:
                            print("⚠️ MoviePy is not available, cannot extract video frames, skipping video input")
                        else:
                            # Not a video, process as an image
                            content.append({
                                "type": "image_url",
                                "image_url": {
                                    "url": f"data:image/jpeg;base64,{video_data}"
                            ...
                            logger.info("✅ Image data added to the message")
                    except Exception as e:
                        logger.warning(f"Video/image data processing failed: {e}")
            
            # Add video file path processing (if there is a saved video file)
                ...
                if video_frames:
                    ...
                    print("⚠️ Video frame extraction failed, skipping video input")
            elif video_path and os.path.exists(video_path) and not MOVIEPY_AVAILABLE:
                print("⚠️ MoviePy is not available, cannot extract video frames")
            
            # Add image (if available and no video)
                    ...
                    if isinstance(audio_data, str):
                        ...
                        if not audio_data.startswith('{') and len(audio_data) > 1000:
                            try:
                                ...
                                logger.info("✅ Base64 audio data added to the message")
                                
                            except Exception as audio_error:
                                logger.warning(f"Audio data processing failed: {audio_error}")
                                # Process as text
                                messages[1]["content"][0]["text"] += f"\n\n🎤 AUDIO INFO: {audio_data[:200]}..."
                        
                        elif audio_data.startswith('{'):
                            # JSON format audio guidance
                            try:
                                ...
                                if audio_json.get("guidance"):
                                    ...
                                    # If there are team communications, add them too
                                ...
                                logger.info("✅ Audio guidance added as text")
                            except json.JSONDecodeError as e:
                                logger.warning(f"Audio JSON parsing failed: {e}")
                        else:
                            # Plain text audio information
                            messages[1]["content"][0]["text"] += f"\n\n🎤 AUDIO INFO: {audio_data}"
                            logger.info("✅ Audio text information added")
    ...
    def _query_model(self, observation, step: int) -> Tuple[str, np.ndarray, Dict]:
        ...
        try:
            ...
            # 🎯 Modified: Remove video modality, as we have converted the video into an image sequence
            payload = {
                "model": self.model_name,
                "messages": messages,
                "modalities": ["text", "audio"],  # Remove video, as it has been converted to images
            ...
            try:
                ...
                error_msg = f"API request exception: {e}"
                logger.error(error_msg)
                return error_msg, np.array([0, 0, 50, 50], dtype=np.int32), media_paths
    ...
    def _create_env_for_episode(self, episode_idx: int) -> CoopCommandGymEnv:
        """Create a new environment instance for the specified episode"""
        try:
            # Calculate the seed for this episode: base seed + episode index, ensuring each episode is independent
            episode_seed = self.seed_index + episode_idx * 1000
            
            # Determine the recording directory
            ...
            env = CoopCommandGymEnv(
                ...
                seed_index=episode_seed,  # Use the calculated episode seed
    ...
    def run_evaluation(self) -> Dict:
        ...
        for episode_idx in range(self.num_episodes):
            ...
            try:
                ...
                # Create a result for the failed episode
                episode_result = {
                ...
                # Clean up the current episode's environment
                if self.env:
                    try:
                        self.env.close()
                    except:
    ...
    def _run_single_episode(self, episode_idx: int) -> Dict:
        ...
        # Reset tracking variables for the current episode
        ...
        while True:
            ...
            # Get the media save directory for the current episode
            ...
            # Temporarily modify save directories
            orig_dirs = None
            if self.save_media:
            ...
            try:
                ...
                # Restore directories (if an error occurred)
                if orig_dirs:
                    self.images_dir, self.audio_dir, self.videos_dir, self.responses_dir = orig_dirs
                raise
    ...
    def _calculate_summary_stats(self, episode_results: List[Dict]) -> Dict:
        """Calculate summary statistics for all episodes"""
        ...
        return {
            ...
            # Consistency metric (inverse of score standard deviation)
            "consistency": 1.0 - (np.std(scores) / 100.0),  
        }

    def _log_final_report(self, summary_stats: Dict, overall_command_compliance: Dict):
        """Output the final evaluation report"""
        logger.info(f"\n{'='*80}")
    ...
    def _convert_numpy_types(self, obj):
        """Recursively convert NumPy types to native Python types to make them JSON serializable"""
